 ...
 */
static switch_status_t http_cache_file_delete(switch_file_handle_t *handle, const char *path)
{
	http_profile_t *profile;
	cached_url_t *u = NULL;
	char *filename = NULL;
	switch_core_session_t *session = NULL;
	filename = switch_core_strdup(gcache.pool, path);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,switch_core_sprintf(gcache.pool, "path is %s\n", path));
	u = cached_url_create(&gcache, path, NULL);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,switch_core_sprintf(gcache.pool, "File is %s\n", filename));
	if (switch_file_exists(u->filename, gcache.pool) == SWITCH_STATUS_SUCCESS) {
		cached_url_destroy(u, gcache.pool);
	}
